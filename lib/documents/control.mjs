 ...
 */
export function issueProblems(revisions, next, { approvalRequired = false } = {}) {
  const out = [];
  if (!PURPOSES[next.purpose]) out.push("هدف صدور را انتخاب کنید");
  if (!String(next.revision || "").trim()) out.push("شمارهٔ رویژن لازم است");
  if (!next.issuedOn) out.push("تاریخ صدور لازم است");
  if (revisions.some((r) => r.revision === next.revision)) out.push(`رویژن ${next.revision} پیش‌تر صادر شده؛ رویژن تازه بدهید`);
  const last = revisions[revisions.length - 1];
  if (last?.purpose === "VOID") out.push("مدرک باطل شده؛ رویژنی پس از آن صادر نمی‌شود");
  if (last && next.issuedOn && String(next.issuedOn) < String(last.issuedOn))
    out.push(`تاریخ صدور پیش از رویژن قبلی (${last.issuedOn}) است — صدور به گذشته برده نمی‌شود`);
  if (next.purpose === "IFC" && approvalRequired && !revisions.some((r) => r.approvedCode === 1 || r.approvedCode === 2))
    out.push("این مدرک تأیید کارفرما لازم دارد؛ تا رویژنی با کد ۱ یا ۲ برنگشته، IFC صادر نمی‌شود");
  return out;
}
