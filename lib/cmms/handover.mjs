 ...
 *
 * DESIGN CONDITIONS (pressure, temperature) and DCS/HISTORIAN TAGS are
 * recorded alongside the nameplate but are not part of readiness: they are
 * useful to operations and MOC, not a precondition the CMMS enforces before
 * accepting the tag. See designConditionProblems and dcsPointProblems below.
 */

/**
 * Design conditions come from the equipment datasheet — a number an
 * engineer reads off a PDF, never inferred. The platform will not take one
 * without knowing which datasheet: a value needs a datasheet revision, and
 * a datasheet revision cited for nothing is refused too (it says which
 * document but not what it is being used for). `revisionTagId` is the
 * tag_id of the mdr_document the cited revision belongs to: undefined when
 * the revision is not on record at all, null when its document is
 * registered against no tag — checked against this tag's id.
 */
export function designConditionProblems({ designPressureBarg, designTempMinC, designTempMaxC, datasheetRevisionId }, { revisionTagId, tagId } = {}) {
  const out = [];
  const has = (v) => v !== null && v !== undefined && String(v).trim() !== "";
  const anyValue = has(designPressureBarg) || has(designTempMinC) || has(designTempMaxC);
  if (anyValue && !has(datasheetRevisionId)) out.push("شرایط طراحی بدون ارجاع به رویژن دیتاشیت ثبت نمی‌شود");
  if (has(datasheetRevisionId) && !anyValue) out.push("رویژن دیتاشیت بدون هیچ مقدار طراحی معنا ندارد");
  if (has(datasheetRevisionId)) {
    if (revisionTagId === undefined) out.push("این رویژن در رجیستر مدارک نیست");
    else if (revisionTagId === null) out.push("مدرک این رویژن در رجیستر به هیچ تگی وصل نشده — اول آن را به همین تگ وصل کنید");
    else if (revisionTagId !== tagId) out.push("این رویژن دیتاشیت تگ دیگری است");
  }
  for (const [v, label] of [[designPressureBarg, "فشار طراحی"], [designTempMinC, "کمینهٔ دمای طراحی"], [designTempMaxC, "بیشینهٔ دمای طراحی"]]) {
    if (has(v) && !Number.isFinite(Number(v))) out.push(`${label} عدد نیست`);
  }
  if (has(designTempMinC) && has(designTempMaxC) && Number.isFinite(Number(designTempMinC)) && Number.isFinite(Number(designTempMaxC))
      && Number(designTempMinC) > Number(designTempMaxC)) out.push("کمینهٔ دمای طراحی از بیشینه بیشتر است");
  return out;
}

/**
 * A DCS/Historian point: what it measures, with its identifier in at least
 * one of the two systems. This is a registry, not a live feed — the
 * platform records where the point can be found, nothing more.
 */
export function dcsPointProblems({ label, dcsTag, historianTag }) {
  const out = [];
  const has = (v) => v !== null && v !== undefined && String(v).trim() !== "";
  if (!has(label)) out.push("عنوان پارامتر لازم است");
  if (!has(dcsTag) && !has(historianTag)) out.push("شناسهٔ DCS یا Historian — دست‌کم یکی — لازم است");
  return out;
}
