 ...
 * credit (lib/controls/evm.mjs) weights them. For two disciplines they come
 ...
 *
 * For the other four the milestones are DERIVED — a pour judged against
 ...
 *
 *   piping      weld · welded · NDT complete as its requirement defines it:
 *               its own accepted examination at 100%; for a random
 *               percentage, its welder's sample in the line's draw, with
 *               B31.3 §341.3.4 progressive examination (lib/ndt/compliance.mjs,
 *               F-17 — before it, a weld outside the sample was never
 *               "tested"; F-16 before that counted a rejected one)
...
export const SQL_DISCIPLINES = {
  electrical: "کابل‌ها", instrumentation: "ابزارها",
};
export const ENGINE_DISCIPLINES = {
  piping: { title: "جوش‌های پایپینگ", installed: "welded", tested: "ndt" },
