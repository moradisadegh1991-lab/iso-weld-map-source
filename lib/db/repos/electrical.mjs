...
export async function recordCableActivity(db, {
  ...
  await assertStepReleased(db, { projectId, scope: "cable", stepCode: code, itemKind: "cable", itemId: cableId });
  const { rows } = await db.query(
 ...
 */
export async function deriveTagElectrical(db, { projectId, tagId, loader = null }) {
  const rows = loader ? await loader.cableIds(tagId) : (await db.query(
