 ...
 */
export async function deriveFoundationSteps(db, { projectId, tagId, today = new Date() }) {
  const notes = {};
  const { rows: [spec] } = await db.query(
    "SELECT * FROM foundation_spec WHERE tag_id = $1 AND project_id = $2", [tagId, projectId]);
  const { rows: pours } = await db.query(
  ...
  for (const p of pours) {
    ...
    const cls = await classEvaluation(db, { projectId, concreteClass: p.concrete_class,
