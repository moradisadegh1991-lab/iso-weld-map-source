...
export async function packageStatus(db, { projectId, packageId }) {
  ...
  const { rows: lines } = await db.query(
    `SELECT l.id, l.line_no, l.piping_class,
            ...
            -- Examined AND accepted: fact_weld.is_examined counts any first
            -- NDT record, pending or rejected ones included, which is right
            -- for progress and wrong for "ready to pressurise". Here a weld
            -- counts when every record of its latest repair cycle is accepted.
            (SELECT count(*) FROM reporting.fact_weld f
              WHERE f.line_key = l.id AND EXISTS (
                SELECT 1 FROM ndt_record r WHERE r.project_id = l.project_id AND r.weld_uid = f.weld_uid::text)
                AND NOT EXISTS (
                SELECT 1 FROM ndt_record r WHERE r.project_id = l.project_id AND r.weld_uid = f.weld_uid::text
                   AND r.repair_cycle = (SELECT max(r2.repair_cycle) FROM ndt_record r2
                                          WHERE r2.project_id = l.project_id AND r2.weld_uid = f.weld_uid::text)
                   AND r.result <> 'accept'))::int AS examined,
...
export async function subsystemMc(db, { projectId, subsystemId, blocks = null }) {
...
export async function mcBoard(db, { projectId }) {
  ...
  const out = [];
  for (const s of subs) {
    const r = await subsystemMc(db, { projectId, subsystemId: s.id, blocks });
    out.push({ ...s, ...r,
