...
/** Record that a spool has been built, tested, shipped. Evidence, so it is attributed. */
export async function setSpoolStatus(db, { projectId, spoolId, status, note = null, userId = null }) {
  const { rows } = await db.query(
    `UPDATE spool
        SET fab_status = $1, fab_status_at = now(), fab_status_by = $2, fab_note = $3
      WHERE id = $4 AND project_id = $5
      RETURNING id, spool_no, fab_status, fab_status_at`,
    [status, userId, note, spoolId, projectId]);
  if (!rows[0]) throw Object.assign(new Error("spool not found"), { status: 404 });
  return rows[0];
}
