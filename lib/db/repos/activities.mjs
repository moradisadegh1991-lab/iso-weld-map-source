 ...
 */
import { deriveJointNdt } from "./joint-ndt.mjs";
import {
...
export async function recordActivity(db, {
  ...
  // A Hold or Witness of the project's approved ITP on this step: released first.
  if (status === DONE) await assertStepReleased(db, { projectId, scope: tag.kind, stepCode: code, itemKind: "tag", itemId: tagId });
...
const GROUPS = [
  ...
  { codes: new Set(["weld_ndt"]), key: "jointNdt", load: deriveJointNdt },
];
