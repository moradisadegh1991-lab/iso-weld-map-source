...
export async function recordActivity(db, {
  ...
  // answered (a pour, its curing, its strength); others are answered only
  ...
  if (step?.derive) {
    const sourced = ALWAYS_DERIVED.has(step.derive)
      || (await DERIVERS[step.derive]?.(db, { projectId, tagId })) !== undefined;
    if (sourced) {
...
export async function tagStatus(db, { projectId, tagId }) {
  ...
  for (const s of chain.filter((x) => x.derive)) {
    let v;
    if (FOUNDATION_STEPS.has(s.derive)) {
      civil ||= await deriveFoundationSteps(db, { projectId, tagId });
      v = civil.steps[s.derive];
      if (civil.notes[s.derive]) notes[s.code] = civil.notes[s.derive];
    } else {
      v = await DERIVERS[s.derive]?.(db, { projectId, tagId });
    }
...
const FOUNDATION_STEPS = new Set(["pour", "curing", "strength"]);
const ALWAYS_DERIVED = FOUNDATION_STEPS;
...
const DERIVERS = { piping: derivePiping, foundation: deriveEquipmentFoundation };
