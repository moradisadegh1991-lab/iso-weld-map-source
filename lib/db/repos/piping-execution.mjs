...
export async function spoolStatus(db, { projectId, spoolId }) {
  const { rows: [spool] } = await db.query(
    `SELECT s.id, s.spool_no, s.length_mm, s.nps_max, s.fab_status, l.line_no
  ...
  const steps = walk(CHAIN, recorded).map((s) => {
    ...
    return { ...s, na: na.has(s.code), doneAt: a?.done_at || null, refNo: a?.ref_no || null };
  });
...
export async function spoolBoard(db, { projectId, lineId = null }) {
  ...
  for (const { id } of rows) {
    ...
    out.push({
      ...
      pct: s.progress.pct, ready: s.why.ready, buried: s.buried,
