 ...
 */
import { supportJointsOpen } from "./joint-ndt.mjs";
import {
...
export function deriveFromWelds(welds) {
  ...
  return {
    ...
    // NDT is DONE when it is ACCEPTED — every method at its latest repair
    // cycle — and STARTED when a film is shot. A rejected or pending shot
    // is work under way, not work done (review finding F11).
    shop_ndt: state(shop, (w) => w.is_accepted, (w) => w.is_examined),
    // Field welding counts as DONE only when the weld is made AND accepted:
    // the next step is the pressure test, and a joint that has not passed
    // is not ready to be tested. It has STARTED as soon as one is made.
    field_weld: state(field, (w) => w.is_welded && w.is_accepted, (w) => w.is_welded),
  };
}
...
export async function spoolStatus(db, { projectId, spoolId }) {
  ...
  const { rows: welds } = await db.query(
    `SELECT is_field_weld, is_welded, is_examined, is_accepted, is_buried
...
export async function markSupport(db, { projectId, supportId, step, on, userId = null }) {
  ...
  if (step === "inspected") {
    ...
    const open = await supportJointsOpen(db, { projectId, supportId });
    if (open.length) throw bad(`NDT جوش‌های اتصال این ساپورت کامل نیست: ${open.map((j) => `${j.jointNo} (${j.text})`).join("، ")}`);
    const { rows } = await db.query(
