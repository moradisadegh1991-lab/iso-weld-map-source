...
export async function recordCoatingActivity(db, {
  ...
  await assertStepReleased(db, { projectId, scope: "coating", stepCode: code, itemKind: "coating_item", itemId: itemId });
  const { rows } = await db.query(
 ...
 */
export async function deriveCoatingFor(db, { projectId, spoolId = null, tagId = null, loader = null }) {
  const item = loader && tagId && !spoolId ? await loader.coatingItem(tagId) : (await db.query(
    `SELECT id, insulation FROM coating_item
      WHERE project_id = $1 AND (spool_id = $2::uuid OR tag_id = $3::uuid)`, [projectId, spoolId, tagId])).rows[0];
