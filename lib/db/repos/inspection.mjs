...
export async function resolveItem(db, { projectId, itemKind, itemId }) {
  const q = {
    ...
    lot: `SELECT l.id, ${LOT_LABEL} AS label, NULL::uuid AS subsystem_id, 'material' AS scope
...
export async function itemsForScope(db, { projectId, scope }) {
  ...
  const q = {
    ...
    lot: [`SELECT l.id, ${LOT_LABEL} AS label FROM material_lot l JOIN material_item mi ON mi.id = l.item_id
