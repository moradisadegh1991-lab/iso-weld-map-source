...
export async function addMrLine(db, { projectId, mrId, itemId = null, tagId = null, description = null, qty, uom = null, needOn = null }) {
  ...
  const { rows: [l] } = await db.query(
    `INSERT INTO mr_line (project_id, mr_id, line_no, item_id, tag_id, description, qty, uom, need_on)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
    [projectId, mrId, n + 1, itemId || null, tagId || null, blank(description), Number(qty), unit, needOn || null]);
  return l;
}
...
export async function awardMr(db, { projectId, mrId, bidId, poNo, placedOn, justification = null, userId = null, today = todayIso() }) {
  ...
  for (const l of rev.lines) {
    await addPoLine(db, { projectId, poId: po.id, itemId: l.item_id, tagId: l.tag_id, description: l.description,
      qty: l.qty, uom: l.uom, needOn: l.need_on });
  }
  await db.query(
...
async function mrLines(db, projectId, mrId) {
  const { rows } = await db.query(
    `SELECT l.id, l.line_no, l.item_id, l.tag_id, l.description, l.qty::float8 AS qty, l.uom, l.need_on,
