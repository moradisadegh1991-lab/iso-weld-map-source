...
export async function missingInformation(db, { projectId }) {
  ...
  // Safe work (047): each of these is a rule only the project can state.
  for (const [kind, setting, title] of [["scaffold", "hse_scaffold_inspection_days", "فاصلهٔ بازرسی داربست (روز)"],
                                         ["crane", "hse_crane_inspection_days", "فاصلهٔ بازرسی جرثقیل / بالابر (روز)"]]) {
    if (p[setting] != null) continue;
    add(`hse-${kind}-interval`, (await q(`SELECT count(*)::int AS n FROM hse_equipment WHERE project_id = $1
        AND kind = '${kind}' AND dismantled_on IS NULL`)).n,
      { title, discipline: "HSE", impact: "high",
        why: "هیچ‌کدام «سالم تا تاریخ …» نمی‌گیرد و مجوزی که به آن‌ها اشاره کند صادر نمی‌شود", unit: "تجهیز", fixAt: "/project" });
  }
  if (p.hse_risk_max_residual == null) {
    add("hse-risk-max", (await q("SELECT count(*)::int AS n FROM hse_jsa WHERE project_id = $1 AND status = 'draft'")).n,
      { title: "حداکثر ریسک باقیماندهٔ پذیرفتنی (JSA)", discipline: "HSE", impact: "high",
        why: "هیچ JSA ای تأیید نمی‌شود", unit: "JSA", fixAt: "/project" });
  }
  // Permit types in use with no competence rule stated for them.
  const types = (await db.query("SELECT DISTINCT type FROM hse_permit WHERE project_id = $1", [projectId])).rows.map((r) => r.type);
  const rules = p.hse_competence_rules;
  add("hse-competence-rule", types.filter((t) => !rules || !(t in rules)).length,
    { title: "صلاحیت لازم برای هر نوع مجوز", discipline: "HSE", impact: "medium",
      why: "صلاحیت خدمهٔ این نوع مجوزها سنجیده نمی‌شود", unit: "نوع مجوز", fixAt: "/hse" });
  if (p.hse_jsa_required_types == null) {
    add("hse-jsa-rule", types.length, { title: "انواع مجوزی که JSA لازم دارند", discipline: "HSE", impact: "medium",
      why: "لزوم JSA برای مجوزها سنجیده نمی‌شود", unit: "نوع مجوز", fixAt: "/hse" });
  }
  add("hse-card-expiry", (await q(`SELECT count(*)::int AS n FROM hse_competence WHERE project_id = $1
      AND expires_on IS NULL AND NOT no_expiry AND revoked_on IS NULL`)).n,
    { title: "تاریخ انقضای کارت صلاحیت", discipline: "HSE", impact: "medium",
      why: "کارت بدون تاریخ انقضا معتبر شمرده نمی‌شود", unit: "کارت", fixAt: "/hse" });
