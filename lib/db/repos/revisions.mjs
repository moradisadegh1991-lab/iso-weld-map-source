...
export async function compareRuns(db, { projectId, fromRunId, toRunId }) {
  ...
  // Whether steel exists comes from the OLD revision: those are the spools
  // that physically exist. The new revision's spools are, by definition,
  // paper. `built` is derived (reporting.spool_stage) — any weld made or any
  // physical step recorded — never a status someone remembered to set.
  const stageBySpool = new Map();
  for (const r of before.rows) {
    if (r.spool_no) {
      stageBySpool.set(r.spool_no, { stage: r.spool_stage || "planned", built: !!r.spool_built });
    }
  }
  ...
  return {
    ...
    diff,
    impact: spoolImpact(diff, stageBySpool),
  };
}
