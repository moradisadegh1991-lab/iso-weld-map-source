...
export const NOT_HELD = [
  ...
  { key: "cmms", title: "برنامهٔ PM و BOM قطعات یدکی (CMMS)" },
  { key: "inst_asset", title: "ابزار دقیق به‌عنوان asset مستقل (نه فقط اتصال به تگ تجهیز)" },
];
...
export async function assetThread(db, { projectId, tagId = null, tagNo = null }) {
  ...
  // ── quality: punch items and NCRs on this tag ──
  const { rows: punch } = await db.query(
    `SELECT id, punch_no, category, status, description, due_on FROM punch_item
      WHERE project_id = $1 AND tag_id = $2 ORDER BY (status = 'closed'), category, punch_no`, [projectId, tag.id]);
  const { rows: ncrs } = await db.query(
    `SELECT id, ncr_no, title, severity, status FROM ncr
      WHERE project_id = $1 AND tag_id = $2 ORDER BY (status = 'closed'), ncr_no`, [projectId, tag.id]);

  // ── procurement: the PO lines buying this tag ──
  const { rows: purchase } = await db.query(
    `SELECT f.po_line_key AS id, f.po_no, f.vendor_code, f.promised_on, f.forecast_on, f.need_on, f.fat_required,
            f.received_qty::float8 AS received_qty, f.qty::float8 AS qty, f.float_days
       FROM reporting.fact_po_line f WHERE f.project_key = $1 AND f.tag_key = $2 ORDER BY f.po_no`, [projectId, tag.id]);

  // ── maintenance: the asset master for the CMMS ──
  const { rows: [master] } = await db.query(
    `SELECT m.iso_class, m.criticality, m.manufacturer, m.model, m.serial_no,
            m.design_pressure_barg, m.design_temp_min_c, m.design_temp_max_c,
            dd.doc_no AS datasheet_doc_no, dr.revision AS datasheet_revision
       FROM asset_master m
       LEFT JOIN mdr_revision dr ON dr.id = m.design_datasheet_rev_id
       LEFT JOIN mdr_document dd ON dd.id = dr.mdr_id
      WHERE m.project_id = $1 AND m.tag_id = $2`,
    [projectId, tag.id]);
  const { rows: dcsPoints } = await db.query(
    "SELECT label, dcs_tag, historian_tag, uom FROM asset_dcs_tag WHERE project_id = $1 AND tag_id = $2 ORDER BY label",
    [projectId, tag.id]);
  ...
  return {
    tag, status, foundations, lines, cables, instruments, coating, packages, punch, ncrs, purchase, master: master || null,
    dcsPoints,
