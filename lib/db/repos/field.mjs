...
import { AREAS, areaForOp, inArea } from "../../platform/areas.mjs";
...
import { irBoard, recordResult, holdsFor } from "./inspection.mjs";

const BEFORE_LOOP = ["calibrated", "installed", "hookup", "wired"];
// A step not yet done that the project's ITP holds carries the hold: which
// row, and where its request stands.
const stepsOf = (steps, hold = () => null) => steps.map((x) => ({ code: x.code, title: x.title, status: x.status, derived: !!x.derived, na: !!x.na,
  hold: x.status !== "done" && !x.derived && !x.na ? hold(x.code) : null }));
...
export async function fieldPack(db, { projectId, subsystemId = null }) {
  ...
  const holds = await holdsFor(db, { projectId });
  ...
  for (const t of tagRows) {
    ...
    tags.push({
      ...
      steps: s.steps ? stepsOf(s.steps, (code) => holds(t.kind, "tag", t.id, code)) : null,
  ...
  for (const { id } of spoolRows) {
    ...
    spools.push({ id, no: s.spool.spool_no, line: s.spool.line_no, pct: s.progress.pct, ready: !!s.why.ready, stage: s.headline,
      steps: stepsOf(s.steps, (code) => holds("piping_spool", "spool", id, code)),
  ...
  for (const { id } of cableRows) {
    ...
    cables.push({ id, no: s.cable.cable_no, description: [s.cable.from_ref, s.cable.to_ref].filter(Boolean).join(" → "),
      ...
      pct: s.progress.pct, ready: !!s.why.ready, steps: stepsOf(s.steps, (code) => holds("cable", "cable", id, code)), irRequirement: s.ir.requirement,
  ...
  for (const { id } of instRows) {
    ...
    instruments.push({ id, no: i.tag_no, description: i.service || null, tag: i.eq_tag_no, category: i.category, loopNo: i.loop_no,
      ...
      pct: s.progress.pct, ready: !!s.why.ready, steps: stepsOf(s.steps, (code) => holds("instrument", "instrument", id, code)),
      calRequirement: s.calibration.requirement });
  }
  ...
  // Inspections awaiting a result on the packed items: an inspector signs
  // at the item, network or not. The state is the server's reading; a
  // signature queued on the phone is shown beside it, not folded into it.
  const inspections = (await irBoard(db, { projectId, state: "awaiting" }))
    .filter((r) => !subsystemId || r.subsystem_id === subsystemId)
    .map((r) => ({ id: r.id, irNo: r.ir_no, itemKind: r.item_kind, itemId: r.item_id, itemLabel: r.itemLabel,
      itpNo: r.itp_no, seq: r.seq, title: r.activity.title, criteria: r.activity.criteria, reference: r.activity.reference,
      plannedAt: r.planned_at, location: r.location, parties: r.state.parties.map((x) => ({ party: x.party, point: x.point, outcome: x.outcome })) }));
  ...
  return {
    ...
    tags, spools, cables, instruments, loops: Object.values(loops), punch, inspections,
  };
}
...
export async function applyOps(db, { projectId, userId, membership, ops = [], today = null, store = null }) {
  ...
  for (const op of inCaptureOrder(ops)) {
    ...
    if (problems.length) error = problems.join(" · ");
      ...
      // The member's work areas, per operation: a batch from one phone can
      // hold a piping step and a punch item, and each is judged on its own.
      let tagKind = null;
      if (op.kind === "tag_step") {
        tagKind = (await withProject(db, projectId, async () =>
          (await db.query("SELECT kind FROM tag WHERE id::text = $1 AND project_id = $2", [String(op.payload?.tagId), projectId])).rows[0]))?.kind || null;
      }
      const area = areaForOp(op.kind, { tagKind });
      if (!inArea(membership, area)) error = `ثبت در بخش «${AREAS[area]}» در حوزهٔ کاری شما نیست`;
    }

    if (!error) {
      try {
        const result = await db.transaction((tx) => withProject(tx, projectId, async () => {
          const r = await APPLY[op.kind](tx, { projectId, userId, p: op.payload, store, membership, capturedAt: op.capturedAt });
          await tx.query(
...
const APPLY = {
  ...
  async ir_result(db, { projectId, userId, p, membership, capturedAt }) {
    // Signed when it was signed on site — never later than it reached us.
    const at = new Date(Math.min(Date.parse(capturedAt), Date.now()));
    const r = await recordResult(db, { projectId, irId: p.irId, outcome: p.outcome, aboutParty: p.aboutParty || null,
      inspectorName: p.inspectorName || null, comments: p.comments || null, membership, userId, now: at });
    return { irId: p.irId, party: r.result.party, state: r.state.state };
  },
