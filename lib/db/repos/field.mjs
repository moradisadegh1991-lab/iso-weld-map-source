...
import { OP_KINDS, opProblems, inCaptureOrder } from "../../field/ops.mjs";
...
export async function fieldPack(db, { projectId, subsystemId = null }) {
  ...
  const { rows: cables } = await db.query(
    `SELECT f.cable_key AS id, f.cable_no AS no, f.is_pulled, f.is_ready, t.tag_no AS tag
       FROM reporting.fact_cable f LEFT JOIN tag t ON t.id = f.tag_key
      WHERE f.project_key = $1 AND ($2::uuid IS NULL OR f.subsystem_key = $2) ORDER BY f.cable_no`, scope);
  const { rows: instruments } = await db.query(
    `SELECT f.instrument_key AS id, f.tag_no AS no, f.loop_no, f.is_installed, f.is_loop_checked, f.is_ready, t.tag_no AS tag
       FROM reporting.fact_instrument f LEFT JOIN tag t ON t.id = f.tag_key
      WHERE f.project_key = $1 AND ($2::uuid IS NULL OR f.subsystem_key = $2) ORDER BY f.tag_no`, scope);
  ...
  return {
    ...
    tags, spools, cables, instruments, punch,
  };
}
 ...
 */
export async function applyOps(db, { projectId, userId, membership, ops = [], today = null }) {
  ...
  for (const op of inCaptureOrder(ops)) {
    ...
    if (!error) {
      try {
        const result = await db.transaction((tx) => withProject(tx, projectId, async () => {
          const r = await APPLY[op.kind](tx, { projectId, userId, p: op.payload });
          await tx.query(
            `INSERT INTO field_op (op_id, project_id, kind, payload, captured_at, user_id, status, result)
             VALUES ($1,$2,$3,$4,$5,$6,'applied',$7)`,
            [op.opId, projectId, op.kind, JSON.stringify(op.payload), op.capturedAt, userId, JSON.stringify(r)]);
          return r;
        }));
    ...
    await withProject(db, projectId, () => db.query(
      ...
      [op.opId, projectId, OP_KINDS[op.kind] ? op.kind : "unknown", JSON.stringify(op.payload || {}),
