...
export async function authenticatePassword(db, { email, password, source = null }) {
  ...
  const { rows: [row] } = await db.query(
    `SELECT u.id, u.subject, u.email, u.display_name, u.is_active,
  ...
  // Said only to someone who knows the password: a deactivated account.
  if (row.is_active === false) {
    throw Object.assign(new Error("این حساب غیرفعال شده است؛ با مدیر پروژه تماس بگیرید."), { status: 403, code: "ACCOUNT_DISABLED" });
  }
