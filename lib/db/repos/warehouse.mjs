...
import { materialScopeOf } from "../../inspection/itp.mjs";
...
export async function inspectLot(db, { projectId, lotId, qtyAccepted, qtyRejected = 0, mirRef = null, inspectedOn }) {
  ...
  if (a > 0) await assertStepReleased(db, { projectId, scope: materialScopeOf(lot.item_category), stepCode: "mir", itemKind: "lot", itemId: lotId });
  const { rows } = await db.query(
...
export async function reviewMtc(db, { projectId, lotId, mtcRef, heatNo, mtcStatus }) {
  ...
  if (mtcStatus === "accepted") {
    await assertStepReleased(db, { projectId, scope: materialScopeOf(lot.item_category), stepCode: "mtc_review", itemKind: "lot", itemId: lotId });
  }
  const { rows } = await db.query(
...
export async function stock(db, { projectId }) {
  ...
  return lots.map((l) => {
    ...
    return { id: l.id, itemId: l.item_id, code: l.code, description: l.description, category: l.category,
      ...
      itpHold: {
        mir: Number(l.qty_accepted) > 0 ? null : hold(materialScopeOf(l.category), "lot", l.id, "mir"),
        mtc: l.mtc_status === "accepted" ? null : hold(materialScopeOf(l.category), "lot", l.id, "mtc_review"),
      } };
  });
}
...
async function lotRow(db, projectId, lotId) {
  const { rows: [l] } = await db.query(
    `SELECT l.*, mi.category AS item_category FROM material_lot l JOIN material_item mi ON mi.id = l.item_id
      WHERE l.id = $1 AND l.project_id = $2`, [lotId, projectId]);
