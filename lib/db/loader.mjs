  ...
  const loader = {
    ...
    async prime(tagIds) {
      ...
      put("lineCount", new Map((await q(
        ...
        .map((r) => [r.k, r.n])));
      // The tag's piping step: every weld on its lines made and ACCEPTED
      // (fact_weld.is_accepted, F11) — not merely shot, which is what
      // fact_progress credits for progress.
      put("pipingCounts", new Map((await q(
        `SELECT l.tag_id AS k, COUNT(*)::int AS items,
                COUNT(*) FILTER (WHERE f.is_accepted)::int AS tested,
                COUNT(*) FILTER (WHERE f.is_welded)::int AS installed
           FROM reporting.fact_weld f JOIN line l ON l.id = f.line_key
          WHERE f.project_key = $1 AND l.tag_id = ANY($2::uuid[])
          GROUP BY l.tag_id`)).map((r) => [r.k, { items: r.items, tested: r.tested, installed: r.installed }])));
    ...
    pipingCounts: (tagId) => one("pipingCounts", tagId, async () => (await db.query(
      `SELECT COUNT(*)::int AS items,
              ...
              COUNT(*) FILTER (WHERE f.is_welded)::int   AS installed
         FROM reporting.fact_weld f JOIN line l ON l.id = f.line_key
        WHERE f.project_key = $2 AND l.tag_id = $1`, [tagId, projectId])).rows[0]),
