...
const STATIC = [
  ...
  { code: "instrument", discipline: "instrumentation", title: "نصب و تست ابزار دقیق", after: ["set"],
    derive: "instrument" },
  // Field welds on the equipment itself (site-assembled column or tank sections).
  { code: "weld_ndt",   discipline: "equipment",       title: "NDT جوش‌های میدانی تجهیز", after: ["set"],
    derive: "weld_ndt" },
  { code: "ready",      discipline: "equipment",       title: "آمادگی راه‌اندازی",
    after: ["piping", "electrical", "instrument", "weld_ndt"] },
];
...
const FIRED = [
  ...
  // The coil's field welds are examined before the hydrotest pressurises them.
  { code: "weld_ndt",   discipline: "equipment",       title: "NDT جوش‌های میدانی کویل", after: ["coil"],
    derive: "weld_ndt" },
  { code: "coil_test",  discipline: "equipment",       title: "تست هیدرواستاتیک کویل", after: ["coil", "weld_ndt"] },
 ...
 *
 *   PAINT AND FIREPROOFING AFTER WELD NDT. A coat over a field weld hides it
 *   from the visual and surface examinations the NDT matrix asks for.
 *
 * Plumb and bolting are DERIVED: the survey and the bolting records answer
 * them (lib/structural/steel.mjs); weld NDT from the joint register against
 * the project's NDT matrix (lib/ndt/joints.mjs), not applicable when no field
 * weld is registered. Fireproofing does not apply when the
...
const STRUCTURE = [
  ...
  { code: "weld_ndt",     discipline: "structural", title: "NDT جوش‌های میدانی (طبق ماتریس NDT)",
    after: ["erection"], derive: "weld_ndt" },
  ...
  { code: "fireproofing", discipline: "structural", title: "ضدحریق (Fireproofing)",
    after: ["bolting", "weld_ndt"], derive: "fireproofing" },
  { code: "painting",     discipline: "structural", title: "رنگ نهایی و تاچ‌آپ", after: ["bolting", "weld_ndt"],
    derive: "painting" },
...
/**
 * Material receipt: a warehouse lot. Not a construction chain — there is no
 * "ready", the two checks are independent, and both are read off the
 * warehouse's own records (the MIR's quantities, the MTC review), never
 * ticked. So it is not in CHAINS; it is in INSPECTABLE, so that an ITP can
 * name its steps and the inspection gate can hold them like any other.
 *
 * The same two checks answer for every material scope — pressure/structural,
 * electrical, general (lib/inspection/itp.mjs) — because MIR and MTC review
 * are what the warehouse itself records for any lot; what differs between
 * scopes is which activities (and hold points) a project's ITP puts on them,
 * not the steps that exist to be held.
 */
const MATERIAL = [
  { code: "mir",        discipline: "warehouse", title: "بازرسی رسید کالا (MIR): چشمی، ابعادی، شمارش", after: [], derive: "mir" },
  { code: "mtc_review", discipline: "warehouse", title: "بررسی گواهی کارخانه (MTC) و شمارهٔ ذوب", after: [], derive: "mtc" },
];
...
/** The steps an ITP of each kind of work can name: every chain, and material receipt (one per scope). */
export const INSPECTABLE = { ...CHAINS, material_pressure: MATERIAL, material_electrical: MATERIAL, material_general: MATERIAL };
