 ...
 * @param {Map<string,{stage:string,built:boolean}>} stageBySpool
 *        spool_no -> derived stage, from the run being superseded
 */
export function spoolImpact(diff, stageBySpool = new Map()) {
  ...
  // A spool the old run never knew is reported as unknown and NOT built —
  // but it is listed, so an unknown is visible rather than quietly "safe".
  const all = [...touched.values()].map((t) => {
    const st = stageBySpool.get(t.spool);
    return { ...t, stage: st?.stage || "unknown", built: !!st?.built };
  });

  return {
    spools: all,
    // The alarm list: work already done that this revision invalidates.
    rework: all.filter((s) => s.built),
    safe: all.filter((s) => !s.built),
  };
}
