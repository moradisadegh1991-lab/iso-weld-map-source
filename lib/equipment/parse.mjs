...
import { EQUIPMENT_KINDS } from "../platform/precedence.mjs";
...
/**
 * Fired equipment has its own chain (steel, coils, refractory, burners,
 * dry-out) — FIRED in lib/platform/precedence.mjs.
 *
 * "Boiler" is deliberately NOT here. "Boiler Feed Water Pump" is a pump, and
 * with "boiler" in this list it would read two ways and drop to unclassified
 * — a false ambiguity on one of the most common tags in a plant.
 */
const FIRED_WORDS = [
  /\bfurnaces?\b/i, /\bfired[\s-]*heaters?\b/i, /\breformers?\b/i, /کوره/,
];
...
/**
 * @returns {{kind: "rotating"|"static"|"fired"|null, why: string}}
 */
export function classifyKind(...texts) {
  ...
  // More than one kind in one description — "Furnace Feed Pump", "Pump
  // Suction Drum" — is a description this cannot read, not a coin to flip.
  const found = [
    ["rotating", hit(ROTATING_WORDS)], ["static", hit(STATIC_WORDS)], ["fired", hit(FIRED_WORDS)],
  ].filter(([, re]) => re).map(([k]) => k);
  if (found.length > 1) {
    return { kind: null,
      why: `شرح هم‌زمان ${found.map((k) => EQUIPMENT_KINDS[k]).join(" و ")} خوانده می‌شود` };
  }
  if (found.length === 1) return { kind: found[0], why: EQUIPMENT_KINDS[found[0]] };
...
export function summarise({ tags = [] }) {
  const byKind = {
    ...Object.fromEntries(Object.keys(EQUIPMENT_KINDS).map((k) => [k, 0])), unclassified: 0,
  };
