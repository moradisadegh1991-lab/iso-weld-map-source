import { takeOut, od, isWeldedJoint, valveTakeOut, INLINE_BODY } from "./standards";
...
export function buildModel(data, opts = {}) {
  ...
  const to = (nodeI, edge, other) => {
    ...
    if (n.type === "valve-bw") return valveTakeOut(n);
  ...
  const blindValves = nodes.filter((n) => n.type === "valve-bw" && valveTakeOut(n) === 0);
