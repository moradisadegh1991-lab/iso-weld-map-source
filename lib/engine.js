...
export function buildModel(data, opts = {}) {
  ...
  const defaultNps = Number(data?.meta?.nps) || Number(nodes[0]?.nps) || 0;
  const pup = Number(data?.meta?.pupLength) || 0;
  const idx = new Map(nodes.map((n, i) => [n.id ?? String(i), i]));

  // ── edges ──────────────────────────────────────────────────
  let edges = Array.isArray(data?.edges) && data.edges.length
    ? data.edges
        .map((e) => ({ a: idx.get(e.from), b: idx.get(e.to), nps: Number(e.nps) || defaultNps }))
        .filter((e) => e.a != null && e.b != null && e.a !== e.b)
    : nodes.slice(0, -1).map((_, i) => ({ a: i, b: i + 1, nps: Number(nodes[i + 1]?.nps) || defaultNps }));
    ...
    [A, B].forEach((ni) => {
      ...
      for (let i = 0; i < legs.length; i++) {
        for (let j = i + 1; j < legs.length; j++) {
          ...
          E({ kind: "fitting", type: nodes[ni].type, node: ni,
      ...
      if (nodes[ni].type === "tee") {
        ...
        if (run.length === 2) {
          E({ kind: "fitting", type: "tee", node: ni,
  ...
  // only rule needed — it subsumes "long runs get their own spool".
  {
    ...
    let acc = 0, prevEnd = null;
    seqEls.forEach((e) => {
      const l = e.length != null ? e.length : len(sub(e.a, e.b));
      const contiguous = prevEnd && len(sub(e.a, prevEnd)) < WELD_TOL;
      if (!contiguous) acc = 0;
      else if (acc + l > spoolMaxLen) {
        const w = welds.find((q) => len(sub(q.at, e.a)) < WELD_TOL);
