 ...
 */
export function guessMapping(headers, patterns_ = HEADER_PATTERNS) {
  const mapping = {};
  const taken = new Set();
  for (const [field, patterns] of Object.entries(patterns_)) {
    const i = headers.findIndex((h, idx) =>
      !taken.has(idx) && patterns.some((p) => p.test(String(h).trim())));
    if (i >= 0) { mapping[field] = i; taken.add(i); }
  }
  const unmapped = Object.keys(patterns_).filter((f) => !(f in mapping));
