...
export const STATUS = {
  ...
  sample_not_drawn: { done: false, text: "نمونهٔ تصادفی کشیده نشده" },
  ...
  full_examination: { done: false, text: "لات مردود — بازرسی کامل همهٔ جوش‌های لات" },
  extension_unknown: { done: false, text: "نمونه رد شده و گسترش بازرسی در ماتریس NDT تعیین نشده" },
 ...
 */
export function compliance({ welds, selections, records, draws = [], extension = "progressive" }) {
  ...
  for (const w of welds) {
    ...
    if (!lots.has(key)) lots.set(key, lotVerdict({ welds, selected, records, draws, extension, lineId: w.lineId, method: req.method, welderId: w.welderId }));
...
export function lotVerdict({ welds, selected, records, draws = [], extension = "progressive", lineId, method, welderId }) {
  ...
  // What a rejected sample leads to is the governing code's or the specification's: B31.3's
  // progressive examination for piping; for other welds, what the project's NDT matrix states.
  if (extension === "full") return { ...at, status: "full_examination", r0: defects0.length, defects: defects0 };
  if (extension !== "progressive") return { ...at, status: "extension_unknown", r0: defects0.length, defects: defects0 };
