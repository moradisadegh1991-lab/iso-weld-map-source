...
/**
 * Not a project role: a platform flag (app_user.is_developer). A developer's
 * standing in every project is this role, holding every action. It is never
 * assigned from a project's users page.
 */
export const DEVELOPER = "developer";

/** Higher wins when a person somehow holds two. */
const RANK = { viewer: 0, qc: 1, engineer: 2, admin: 3, developer: 4 };
...
export const ACTIONS = {
  ...
  // Procurement: POs, expediting, shipments and the VDRL. A FAT result is a
  // QC record; receiving against a PO is site work, as any receipt is.
  MANAGE_PROCUREMENT: "procurement:manage",
  // Document control: the register, issuing revisions and transmittals.
  CONTROL_DOCUMENTS: "documents:control",
  // Handover to maintenance: the asset master (class, criticality,
  // nameplate) and its export are engineering's.
  MANAGE_HANDOVER: "handover:manage",
  // Inspection: raising requests and signing results is QC's and the
  // inspectors'; which PARTY a person signs for is their membership's
  // (contractor, company or TPI), not their role. Preparing and approving
  // an ITP is engineering's, and the approver is never the preparer.
  RECORD_INSPECTION: "inspection:record",
  MANAGE_ITP: "inspection:itp",
};
...
const GRANTS = {
  ...
  qc: [ACTIONS.VIEW_PROJECT, ACTIONS.UPLOAD_DOCUMENT, ACTIONS.ASSIGN_WELD,
       ...
       ACTIONS.RECORD_QUALITY, ACTIONS.RECORD_COMPLETIONS, ACTIONS.RECORD_INSPECTION],
  engineer: [
    ...
    ACTIONS.RECORD_COMPLETIONS, ACTIONS.SIGN_MC, ACTIONS.MANAGE_PROCUREMENT, ACTIONS.CONTROL_DOCUMENTS,
    ACTIONS.MANAGE_HANDOVER, ACTIONS.RECORD_INSPECTION, ACTIONS.MANAGE_ITP,
  ],
  admin: Object.values(ACTIONS),
  developer: Object.values(ACTIONS),
};
