...
/**
 * Half the face-to-face length of a butt-weld valve, in mm.
 *
 * ASME B16.10 keys face-to-face on valve type and pressure class, neither of
 * which the drawing extraction carries, so the length is read off the drawing
 * into `node.faceToFace` rather than guessed from a table. Without it the
 * valve has no body, both its welds land on the same point and merge — which
 * is why `buildModel` raises a warning instead of silently under-counting.
 */
export function valveTakeOut(node) {
