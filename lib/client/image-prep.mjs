...
export function prepareSource(src) {
  ...
  // The ladder starts at NATIVE resolution and only gives it up if the
  // payload will not fit. It used to start at 1500, which silently threw
  // away a third of the picture on every sheet: an A3 at 300 DPI is
  // 4958x3504, one 3x3 tile covers 2115x1495 of it, and capping the long
  // edge at 1500 scales that by 0.709 — 213 effective DPI. A BOM row is
  // about 2.5 mm of text, so 29 px tall on the render and 21 px by the time
  // the model saw it.
  //
  // Nothing was gained by it. The real project sheet came out at 0.74 MB
  // against a 3.4 MB budget — 22% of the allowance — so the resolution was
  // being discarded to stay inside a limit that was never close.
  //
  // 2200 is above the 2115 a tile actually spans, and renderCrop clamps the
  // scale at 1, so this rung means "do not resample at all". The rungs below
  // are unchanged and still catch a sheet that genuinely will not fit.
  let out = tiles(src, 2200, 0.82, grid);
  if (sum(out) > LIMIT) out = tiles(src, 1500, 0.82, grid);
