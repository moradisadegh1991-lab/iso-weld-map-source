...
import { AREAS, areaForPath, inArea } from "../platform/areas.mjs";
...
export async function authenticate(request, { projectId } = {}) {
  ...
  if (cookie) {
    ...
    const { rows: [user] } = await db.query(
      "SELECT id, subject, email, display_name, is_developer, is_active FROM app_user WHERE id = $1", [claims.sub]);
    if (user && !user.is_active) {
      throw Object.assign(new Error("این حساب غیرفعال شده است."), { status: 401, code: "ACCOUNT_DISABLED" });
    }
    if (!user) {
    ...
    assertAreaWrite(request, membership);
  ...
  if (!user.is_active) throw Object.assign(new Error("این حساب غیرفعال شده است."), { status: 401, code: "ACCOUNT_DISABLED" });
  const membership = projectId ? await membershipOf(db, { projectId, userId: user.id }) : null;
  assertAreaWrite(request, membership);
...
/**
 * A write to a discipline's routes by a member who does not work in it is
 * refused here, once, for every route — rather than in each handler, where
 * one would forget. Reading is not restricted by area (see
 * lib/platform/areas.mjs), and the field sync checks each operation itself.
 */
export function assertAreaWrite(request, membership) {
  if (!membership || ["GET", "HEAD"].includes(String(request?.method || "GET").toUpperCase())) return;
  let path = "";
  try { path = new URL(request.url).pathname; } catch { return; }
  const area = areaForPath(path);
  if (area && !inArea(membership, area)) {
    throw Object.assign(new Error(`ثبت در بخش «${AREAS[area]}» در حوزهٔ کاری شما نیست.`), { status: 403, code: "OUT_OF_AREA" });
  }
}
